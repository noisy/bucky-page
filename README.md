...
The public landing page for Bucky, in English (`/en/`) and Polish (`/pl/`).
...
| Path | What |
|---|---|
| `src/page.html` | The one page template, with `{{key}}` placeholders (`{{key\|url}}` URL-encodes) |
| `src/strings/en.json`, `src/strings/pl.json` | All copy per language. The build fails on a missing or unused key |
...
To add a language: copy `src/strings/en.json` to `<lang>.json`, translate,
